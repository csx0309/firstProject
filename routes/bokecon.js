...
router.get("/show", function(req, res, next) {
    ...
    bokemodel.find({}, function(err, totalData) {
        if (!err) {
            ...
            //pagerData 每页显示的数据
            bokemodel.find({}, function(err, pagerData) {
                ...
                //发送封装的数据到前台
                res.send(resultJson);
            }).sort({ "ctime": -1 }).skip(recordStart).limit(pageSize);
        }
    });
});
//显示单页面的内容
...
router.get("/typecon.html", function(req, res) {
    ...
    bokemodel.find({ tbtype: ty }, function(err, totalData) {
        if (!err) {
            ...
            //db.collection.find().skip(跳过上一页的数据量).limit(每页显示的大小)    可用来做分页，跳过5条数据再取5条数据
            //pagerData 每页显示的数据
            bokemodel.find({ tbtype: ty }, function(err, pagerData) {
                //console.log(pagerData); //默认是第一页的5条数据
...
router.get('/typenum', function(req, res, next) {
    ...
    bokemodel.find({}, function(err, data) {
        if (err) {
            ...
            var shulnum = [{ name: 'HTML相关信息', tp: 'HTML', ttnum: htmlnum }, { name: 'CSS相关信息', tp: 'CSS', ttnum: cssnum }, { name: 'JavaScript相关信息', tp: 'JavaScript', ttnum: jsnum }, { name: 'Java相关信息', tp: 'Java', ttnum: javanum }, { name: '课外知识相关信息', tp: '课外知识', ttnum: kewainum }];
            res.send(shulnum);
        }
    })
});
...
router.get('/search.html', function(req, res, next) {
    ...
    bokemodel.find({ tbcontent: searchcon }, function(err, totalData) {
        if (!err) {
            ...
            //db.collection.find().skip(跳过上一页的数据量).limit(每页显示的大小)    可用来做分页，跳过5条数据再取5条数据
            //pagerData 每页显示的数据
            bokemodel.find({ tbcontent: searchcon }, function(err, pagerData) {
                //console.log(pagerData); //默认是第一页的5条数据
